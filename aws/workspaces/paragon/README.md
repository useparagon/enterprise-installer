...
Set `path_based_routing_enabled = true` to route the supported public services through a shared external host with service-specific path prefixes. The path prefix is taken from each service's existing `*_PUBLIC_URL`; there is no separate path variable.
...
All path-routed services must use the same public host and unique, non-root path prefixes without trailing slashes. AWS ingress rules match the path without requiring the public `Host` header, allowing an upstream enterprise reverse proxy to rewrite `Host`. Route53 continues to expose the standard `<service>.<domain>` origin names for proxy-to-ALB TLS/SNI.

ALB target-group health checks and Kubernetes probes remain on `/healthz` without the public path prefix. Application support is provided separately by PARA-25254; its release must declare `HTTP_PATH_PREFIX` in the affected services' `service-inputs` contract.
...
| [terraform_data.path_based_routing_validation](https://registry.terraform.io/providers/hashicorp/terraform/latest/docs/resources/data) | resource |
...
| [aws_secretsmanager_secret_version.infra_redis](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/secretsmanager_secret_version) | data source |
| [aws_secretsmanager_secret_version.infra_storage](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/secretsmanager_secret_version) | data source |
| [aws_secretsmanager_secret_version.managed_sync](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/secretsmanager_secret_version) | data source |
| [aws_secretsmanager_secret_version.openobserve](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/secretsmanager_secret_version) | data source |
...
| <a name="input_aws_secret_access_key"></a> [aws\_secret\_access\_key](#input\_aws\_secret\_access\_key) | AWS Secret Access Key for AWS account to provision resources on. Null when using ambient credentials (Spacelift AWS integration) with aws\_assume\_role\_arn. | `string` | `null` | no |
...
| <a name="input_docker_password"></a> [docker\_password](#input\_docker\_password) | Docker password to pull images. Null when using a pre-provisioned pull secret (create\_docker\_pull\_secret=false) or when credentials are supplied only in the infra workspace. | `string` | `null` | no |
...
| <a name="input_docker_username"></a> [docker\_username](#input\_docker\_username) | Docker username to pull images. Null when using a pre-provisioned pull secret (create\_docker\_pull\_secret=false) or when credentials are supplied only in the infra workspace. | `string` | `null` | no |
...
| <a name="input_hoop_reviewers_access_groups"></a> [hoop\_reviewers\_access\_groups](#input\_hoop\_reviewers\_access\_groups) | Reviewer groups required for customer-facing app connections. | `list(string)` | <pre>[<br/>  "dev-team-managers"<br/>]</pre> | no |
...
| <a name="input_migrated_workspace"></a> [migrated\_workspace](#input\_migrated\_workspace) | Optional existing workspace name to reuse when reading Secrets Manager handoff secrets (instead of deriving paragon-<org>-<hash>). | `string` | `null` | no |
...
| <a name="input_openobserve_email"></a> [openobserve\_email](#input\_openobserve\_email) | Deprecated. OpenObserve credentials are generated by the infra workspace in Secrets Manager (paragon/<workspace>/openobserve) and synced via ESO. | `string` | `null` | no |
| <a name="input_openobserve_password"></a> [openobserve\_password](#input\_openobserve\_password) | Deprecated. OpenObserve credentials are generated by the infra workspace in Secrets Manager (paragon/<workspace>/openobserve) and synced via ESO. | `string` | `null` | no |
...
| <a name="input_path_based_routing_enabled"></a> [path\_based\_routing\_enabled](#input\_path\_based\_routing\_enabled) | Enable shared-host path-based public routing for supported services. When enabled, supported *\_PUBLIC\_URL values must include a non-root path prefix. | `bool` | `false` | no |
