...
variable "migrated_workspace" {
  description = "Override the workspace name to preserve naming conventions when migrating from legacy workspaces. Must match the infra workspace value; reused when reading Secrets Manager handoff secrets (instead of deriving paragon-<org>-<hash>)."
  type        = string
  default     = null
}
