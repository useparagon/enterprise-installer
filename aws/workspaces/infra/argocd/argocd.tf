    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            ...
            "set -euo pipefail",
            "",
            "# Bastion user-data installs aws/kubectl/helm and configures kubeconfig for ubuntu.",
            "# SSM runs as root with a minimal PATH, so wait for tooling then run as ubuntu.",
            "for i in $(seq 1 60); do",
            "  if sudo -u ubuntu -H bash -lc 'command -v aws >/dev/null && command -v kubectl >/dev/null && command -v helm >/dev/null'; then",
            "    break",
            "  fi",
            "  echo \"Waiting for bastion user-data (attempt $i/60)...\"",
            "  sleep 10",
            "done",
            "sudo -u ubuntu -H bash -lc 'command -v aws >/dev/null && command -v kubectl >/dev/null && command -v helm >/dev/null'",
            "",
