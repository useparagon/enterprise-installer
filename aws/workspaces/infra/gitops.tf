...
locals {
  ...
  # IngressClass already exists, set createIngressClassResource=false to avoid an
  # "already exists" conflict. Driven by an explicit var (set per stack) rather than a
  # plan-time cluster read, which blocks the entire plan (and any destroy refresh)
  # whenever the EKS API is unreachable.
