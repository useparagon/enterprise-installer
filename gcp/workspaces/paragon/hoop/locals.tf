locals {
  ...
  # GKE Workload Identity needs no credential env: the metadata server resolves ADC
  # from the pod's ServiceAccount. gcloud still refuses to run without a project,
  # and hoop exec starts the shell with a clean environment.
  workload_identity_env = var.gcp_project_id != null ? {
    "envvar:CLOUDSDK_CORE_PROJECT" = var.gcp_project_id
  } : {}
