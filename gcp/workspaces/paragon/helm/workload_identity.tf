...
# Agent OS pods (namespace/KSA agent-os) impersonate the dedicated Agent OS GSA.
resource "google_service_account_iam_member" "agent_os_workload_identity" {
  ...
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:${data.google_container_cluster.cluster.project}.svc.id.goog[${kubernetes_namespace_v1.agent_os[0].id}/agent-os]"
