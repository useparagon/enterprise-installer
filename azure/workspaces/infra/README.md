...
| <a name="input_k8s_providers_enabled"></a> [k8s\_providers\_enabled](#input\_k8s\_providers\_enabled) | Configure kubernetes/helm/kubectl providers against the AKS API. Defaults to false; set true when destroying a stack that still has GitOps resources in state while argocd\_enabled is false. | `bool` | `false` | no |
