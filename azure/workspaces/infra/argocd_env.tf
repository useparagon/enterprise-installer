...
locals {
  ...
  argocd_storage  = module.storage.blob

  argocd_cloud_storage_type = "AZURE"
  ...
  argocd_infra_env = merge(local.argocd_domain_env, {
    ...
    CLOUD_STORAGE_PUBLIC_URL        = "https://${local.argocd_storage.public_storage_account_name}.blob.core.windows.net"
    CLOUD_STORAGE_PRIVATE_URL       = "https://${local.argocd_storage.name}.blob.core.windows.net"
    CLOUD_STORAGE_MICROSERVICE_USER = local.argocd_storage.name
    CLOUD_STORAGE_MICROSERVICE_PASS = local.argocd_storage.access_key
    CLOUD_STORAGE_PUBLIC_BUCKET     = local.argocd_storage.public_container
