locals {
  ...
  # Secret names are fixed per the Azure Key Vault naming convention.
  # Secrets are created by this module's secrets.tf when local.secrets_ready.
  env_secret_name          = local.secrets_ready ? "env" : null
  docker_cfg_secret_name   = local.secrets_ready ? "docker-cfg" : null
  managed_sync_secret_name = local.secrets_ready && var.managed_sync_config != null ? "managed-sync" : null
  openobserve_secret_name  = local.secrets_ready ? "openobserve" : null
  ...
  gitops_bridge_annotations = merge(
    {
      ...
      "azure_cluster_name"       = var.cluster_name
...
resource "helm_release" "alb_controller" {
  ...
  values = [yamlencode({
    albController = {
      podIdentity = {
        clientID = azurerm_user_assigned_identity.alb_controller[0].client_id
      }
    }
  })]
...
resource "kubectl_manifest" "external_secrets" {
  ...
  # `local.enabled ? ... : {}` ternary broke plan: its branches had inconsistent
  # types (object-with-attrs vs empty object), which also made for_each keys
  # undeterminable at plan time.
